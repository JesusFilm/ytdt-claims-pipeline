...
function normalizeClaimsColumns(row: Record<string, unknown>) {
...
export default async function validateInputCSVs(context: PipelineContext) {
  const errors: string[] = []
  ...
  if (context.files.claims) {
    for (const [source, filePath] of Object.entries(context.files.claims)) {
      ...
      try {
        ...
        errors.push(`claims (${source}): Failed to parse CSV - ${(error as Error).message}`)
  ...
  for (const fileType of ['mcnVerdicts', 'jfmVerdicts'] as const) {
    ...
    try {
      ...
      errors.push(`${fileType}: Failed to parse CSV - ${(error as Error).message}`)
      console.error(`First 500 chars of content:`, csvContent.substring(0, 500))
    }
  }
