...
describe('connect-vpn', () => {
  ...
  beforeEach(() => {
    ...
    ;(spawn as unknown as ReturnType<typeof vi.fn>).mockReturnValue(mockVpnProcess)
    mockMysqlPool = {
      query: vi.fn().mockResolvedValue([{}]),
      end: vi.fn().mockResolvedValue(undefined),
    }
    ;(mysql.createPool as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(mockMysqlPool)
  })
  ...
  it('should connect VPN when SKIP_VPN is false', async () => {
    const connectVPN = (await import('../connect-vpn')).default
    ;(mockVpnProcess.on as ReturnType<typeof vi.fn>).mockImplementation((event, _callback) => {
      if (event === 'error') {
  ...
  it('should handle MySQL connection errors', async () => {
    const connectVPN = (await import('../connect-vpn')).default
    ;(mockMysqlPool.query as ReturnType<typeof vi.fn>).mockRejectedValue(
