...
describe('drive-upload', () => {
  ...
  beforeEach(() => {
    ...
    ;(google.drive as unknown as ReturnType<typeof vi.fn>).mockReturnValue(mockDriveApi)
    ;(google.auth.GoogleAuth as unknown as ReturnType<typeof vi.fn>).mockImplementation(
      () => mockGoogleAuth
    )
    ;(fs.readFile as unknown as ReturnType<typeof vi.fn>).mockClear()
  })

  describe('getOrCreateFolder', () => {
    it('should find existing folder', async () => {
      vi.resetModules()
      ;(mockDriveApi.files.list as ReturnType<typeof vi.fn>).mockResolvedValue({
        data: { files: [{ id: 'existing-folder-id' }] },
      })
      const { getOrCreateFolder } = await import('../drive-upload')
    ...
    it('should create new folder if not found', async () => {
      ...
      ;(mockDriveApi.files.create as ReturnType<typeof vi.fn>).mockResolvedValue({
    ...
    it('should throw error if shared drive not found', async () => {
      const { getOrCreateFolder } = await import('../drive-upload')
      ;(mockDriveApi.drives.list as ReturnType<typeof vi.fn>).mockResolvedValue({
        data: { drives: [] },
      })
  ...
  describe('uploadFile', () => {
    it('should upload file successfully', async () => {
      ...
      const { uploadFile } = await import('../drive-upload')
  ...
  describe('uploadFileWithFallback', () => {
    it('should return upload result on success', async () => {
      ...
      const { uploadFileWithFallback } = await import('../drive-upload')
    ...
    it('should return fallback result on upload failure', async () => {
      ...
      const mockStream = { pipe: vi.fn() }
      ;(createReadStream as unknown as ReturnType<typeof vi.fn>).mockReturnValue(mockStream)
      ;(mockDriveApi.files.create as ReturnType<typeof vi.fn>).mockRejectedValue(
