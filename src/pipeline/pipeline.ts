...
import { env } from '../env'
import backupTables from '../steps/backup-tables'
import connectVPN from '../steps/connect-vpn'
import disconnectVPN from '../steps/disconnect-vpn'
import enrichML from '../steps/enrich-ml'
import exportViews from '../steps/export-views'
import processClaims from '../steps/process-claims'
import processVerdicts from '../steps/process-verdicts'
import uploadDrive from '../steps/upload-drive'
import validateInputCSVs from '../steps/validate-input-csvs'
...
function getPipelineSteps(files: PipelineContext['files']): PipelineStep[] {
  return [
    {
      name: 'connect_vpn',
      fn: connectVPN,
...
export async function runPipeline(
  ...
  let runId: ObjectIdType | null = null
  ...
  try {
    ...
    console.error(`Pipeline failed at ${context.status}:`, (error as Error).message)
      ...
      await collection.updateOne(
        { _id: runId },
        {
          $set: {
            ...
            endTime: new Date(),
            duration: Date.now() - (context.startTime as number),
          },
        }
      )
      await syncRunState(runId)
...
export async function getCurrentPipelineStatus() {
  try {
    ...
    if (isRunning && checkTimeout({ startTime: currentRun.startTime, status: currentRun.status })) {
    ...
    const completedCount =
      currentRun.startedSteps?.filter((s: { status: string }) => s.status === 'completed').length ||
      0
    const progress = Math.round((completedCount / allSteps.length) * 100)

    const steps = allSteps.map((stepName) => {
      const completed = currentRun.startedSteps?.find((s: { name: string }) => s.name === stepName)
...
export async function syncRunState(
  runId: ObjectIdType,
  ...
  // Check for timeout
  if (run.status === 'running' && checkTimeout({ startTime: run.startTime, status: run.status })) {
  ...
  // Check if pipeline can be marked complete
    ...
    const hasRunningSteps = run?.startedSteps?.some(
    ...
    const startedStepNames = (run.startedSteps || []).map((s: { name: string }) => s.name)
    ...
    if (!hasRunningSteps && allStepsStarted && run.status === 'running') {
      ...
      updateFields.duration =
        (completionData.duration as number) || Date.now() - new Date(run.startTime).getTime()
      console.log('Pipeline marked as completed')

      // Update currentStep to the running step
    } else if (hasRunningSteps) {
      const runningStep = run.startedSteps.find(
        (step: { status: string }) => step.status === 'running'
      )
  ...
  if (env.SLACK_BOT_TOKEN && !run.slackNotified) {
    const finalStatus = (updateFields.status || run.status) as string
    if (finalStatus === 'completed' || finalStatus === 'failed' || finalStatus === 'timeout') {
      try {
        const { sendPipelineNotification } = await import('../lib/slack-notifier')
        await sendPipelineNotification(
          runId.toString(),
          finalStatus,
          (updateFields.error || run.error) as string | null,
          (updateFields.duration || run.duration) as number | null,
          run.files,
          run.startTime,
          (updateFields.results || run.results) as Record<string, unknown> | null
        )
        ...
        console.error('Slack notification failed:', (notifError as Error).message)
      }
    }
  }
}
...
export function checkTimeout(run: { startTime: Date | string; status: string }) {
